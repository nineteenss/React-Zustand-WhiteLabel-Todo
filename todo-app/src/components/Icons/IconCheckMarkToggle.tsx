//
//  IconCheckMarkToggle.tsx
//  react-zustand-whitelabel-todo
//
//  Created by Sergey Smetannikov on 29.01.2025
//

import React from "react";

interface IconToggleDefaults {
  isToggled?: boolean; // Current state param type
}

interface IconsDefaults {
  height?: number;
  width?: number;
  color?: string;
}

export const IconCheckMarkToggle: React.FC<IconToggleDefaults> = ({
  isToggled,
}) => {
  return isToggled ? (
    <SvgIconOne color="black" />
  ) : (
    <SvgIconTwo color="black" />
  );
};

const SvgIconOne: React.FC<IconsDefaults> = ({ height, width, color }) => {
  return (
    <svg
...
const SvgIconTwo: React.FC<IconsDefaults> = ({ width, height, color }) => {
  return (
    <svg
      width={width ? width : "24"}
      height={height ? height : "24"}
